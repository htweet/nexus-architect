...
function CanvasToolbar({ containerRef }: { containerRef: React.RefObject<HTMLElement | null> }) {
  ...
  return (
    <div
      className="absolute bottom-5 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 animate-fade-in"
      style={{
        background: '#0d1610',
...
export function Canvas() {
  ...
  return (
    /* Outer wrapper — position:relative so toolbar/perf stay pinned to the
       visible bottom of the canvas area even while the inner <main> scrolls */
    <div className="relative flex-1 flex flex-col overflow-hidden">
      ...
      {/* Toolbar + perf monitor are OUTSIDE the scrollable <main> so they
          stay anchored to the visible bottom regardless of canvas scroll */}
