 ...
 *
 * Mounts inside DndContext. Uses useDndMonitor to track cursor position
 * every drag frame. Renders:
 *   1. Smart guide lines  — 1px dashed blue lines + pixel-distance badges
 *      connecting the dragged ghost to the nearest edges of the hovered element.
 *   2. Fluid insertion line — 2px solid blue line with terminus circles,
 *      showing exactly where between siblings the element will land.
 *   3. Cursor tooltip — fixed W/H and X/Y readout at 10px monospace.
 *
 * All elements are pointer-events-none and fixed-position (viewport coords).
 */
...
export function SmartGuidesOverlay() {
  ...
  useDndMonitor({
    ...
    onDragEnd()    { setM(EMPTY); },
    onDragCancel() { setM(EMPTY); },
  });
  ...
  // ── Ghost box (centred on cursor for measurement reference) ───────────────
  ...
  // ── Smart guide lines (only when we have a reference rect) ───────────────
  type GuideSpec = {
  ...
  if (overRect && activeW > 0 && activeH > 0) {
    ...
    // Left: ghost left ↔ container left
    push(
    ...
    // Right: ghost right ↔ container right
    push(
    ...
    // Bottom: ghost bottom ↔ container bottom
    push(
  ...
  // ── Insertion line (sibling hovers only — no drop: prefix) ───────────────
  ...
  return (
    <>
      ...
      >
        ...
        {/* Fluid insertion line */}
        {insertY !== null && overRect && (
          <g>
            <line
              x1={overRect.left  - 6} y1={insertY}
              x2={overRect.right + 6} y2={insertY}
              stroke="#0D99FF" strokeWidth={2}
            />
            {/* Left terminus circle */}
            <circle
              cx={overRect.left  - 6} cy={insertY}
              r={4}
              fill="none" stroke="#0D99FF" strokeWidth={2}
            />
            {/* Right terminus circle */}
            <circle
              cx={overRect.right + 6} cy={insertY}
              r={4}
              fill="none" stroke="#0D99FF" strokeWidth={2}
            />
          </g>
      ...
      >
        {activeW > 0 && (
          ...
          >
            <span style={{ color: '#0D99FF' }}>W</span>
