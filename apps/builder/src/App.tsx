...
function BuilderApp() {
  ...
  useEffect(() => {
    // cancelled flag prevents Strict Mode's double-invoke from applying
    // the first (discarded) adapter instance — only the second mount's
    // result is used.  Without this, two createMockAdapterContext() calls
    // race and the first may partially initialise state before being replaced.
    let cancelled = false;
    resolveAdapter()
      .then((ctx) => {
        if (!cancelled) setAdapter(ctx);
      })
      .catch((err: unknown) => {
        if (cancelled) return;
