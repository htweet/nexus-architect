...
export default defineConfig({
  ...
  resolve: {
    alias: {
      '@':                        r('./src'),
      // Subpath must be listed before the root alias — Vite tests aliases in order
      '@nexus/core/plugin-api':   r('../../packages/core/src/plugin-api/index.ts'),
  ...
  build: {
    target: 'es2022',
    // Production: hidden sourcemaps (uploaded to error tracker, never served to browser).
    // Dev builds keep inline sourcemaps via `vite build --sourcemap inline`.
    sourcemap: process.env.NODE_ENV === 'production' ? 'hidden' : true,
    // Emit .vite/manifest.json so PHP Enqueue class can resolve hashed chunk names.
    manifest: true,
    emptyOutDir: true,
